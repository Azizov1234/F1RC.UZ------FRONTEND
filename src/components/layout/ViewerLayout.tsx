...
import { Home, Calendar, Trophy, Radio, User, Menu, X, LogOut, Bell } from 'lucide-react';
...
export default function ViewerLayout() {
  ...
  const navItems = [
    { path: '/viewer',            icon: Home,     label: t.dashboard,   exact: true },
    { path: '/viewer/events',     icon: Calendar, label: t.events },
    { path: '/viewer/leaderboard',icon: Trophy,   label: t.leaderboard },
    { path: '/viewer/streams',    icon: Radio,    label: t.streams },
    { path: '/viewer/profile',    icon: User,     label: t.profile },
  ];
        ...
        <div className="flex items-center justify-between p-5 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground font-heading tracking-widest uppercase">Viewer</p>
            </div>
          </div>
            ...
            return (
              <Link key={item.path} to={item.path} onClick={() => setSidebarOpen(false)}
                className={`flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-heading font-semibold uppercase tracking-wider transition-all duration-300 relative active:scale-[0.98]
                  ${active ? 'bg-primary/10 text-white border border-primary/20 shadow-[0_4px_15px_rgba(255,0,0,0.1)]' : 'text-muted-foreground hover:bg-white/5 hover:text-white border border-transparent'}`}>
      ...
      <div className="flex flex-col min-h-screen lg:pl-64">
        <header className="sticky top-0 z-30 bg-background/80 backdrop-blur-md border-b border-border flex items-center justify-between px-4 h-14">
          ...
          <div className="hidden lg:flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span className="font-mono">F1RC.UZ VIEWER</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="w-10 h-10 rounded-xl bg-zinc-500/10 border border-zinc-500/20 flex items-center justify-center text-sm font-heading font-bold text-foreground">{user?.full_name?.[0] || 'V'}</div>
          </div>
        </header>
