...
type RoleKey = Uppercase<UserRole> | 'TEAM_MANAGER';
...
const roleConfig: Record<RoleKey, { label: string; bg: string; text: string; border: string }> = {
  ...
  VIEWER:       { label: 'VIEWER',      bg: 'bg-gray-500/15',   text: 'text-gray-400',   border: 'border-gray-500/30'   },
};
...
export default function RoleBadge({ role }: RoleBadgeProps) {
  ...
  const config = roleConfig[key] ?? roleConfig.VIEWER;
  return (
    <span
